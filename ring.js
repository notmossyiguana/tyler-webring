...
(function () {
  ...
  function parseSiteEntry(site) {
    ...
    if (firstSlash === -1 && firstQ === -1) return { host: s, path: '' };
    if ((firstQ !== -1 && firstSlash === -1) || (firstQ !== -1 && firstQ < firstSlash))
      return { host: s.slice(0, firstQ), path: s.slice(firstQ) };
    return { host: s.slice(0, firstSlash), path: s.slice(firstSlash) };
  }
  ...
  for (var i = 0; i < webring.sites.length; i++) {
    try {
      if (siteMatches(webring.sites[i])) { webring.idx = i; break; }
    } catch (e) {
